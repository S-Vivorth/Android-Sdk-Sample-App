...
import com.oone.paymentSdk.paymentSdk
import com.oone.paymentSdk.payment_confirm
...
class MainActivity : AppCompatActivity() {
    ...
    lateinit var bottomsheetFrag: paymentSdk
...
//            Toast.makeText(this@MainActivity, params.toString(), Toast.LENGTH_SHORT).show()
    payment_confirm(environment = environment, activity = this@MainActivity, language= language).payment_confirm(params[0],""){
                    ...
                    runOnUiThread {
                        kotlin.run {
                            if (sessionId != "null") {
                                 ...
                                 bottomsheetFrag = paymentSdk(
                                    supportFragmentManager = supportFragmentManager,
                                    paylater = pay_later(),
                                    sessionId = "$sessionId",
                                    clientID = "fmDJiZyehRgEbBJTkXc7AQ==",
                                ...
                                Toast.makeText(
                                    applicationContext,
                                    checkoutObject.optString("message"),
                                    Toast.LENGTH_SHORT
                                )
                                    .show()
                            }
...
//        MainActivity().bottomSheetBehavior.state = BottomSheetBehavior.STATE_COLLAPSED
    ...
    fun openAmkDeeplink() {
        ...
        sourceInfo.appIconUrl = "https://admin.oone.com.kh/images/logo/oone_logo_khmer.png"
        sourceInfo.appName = "oone Mobile"
        ...
        amkdlMerchant.merchantName = "oone"
        amkdlMerchant.receiverName = "oone"
    ...
    fun openDeeplink() {
        ...
        try {
            val url =
                "market://com.domain.acledabankqr/app/?creditAccount=123456&paymentAmount=10&paymentAmountCcy=USD&paymentPurpose=purpose desc"
            val intent = Intent(Intent.ACTION_VIEW, Uri.parse(url))
            startActivity(intent)
        } catch (ex: Exception) {
            val package_name = "com.domain.acledabankqr"
